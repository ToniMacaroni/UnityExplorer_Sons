...
namespace UnityExplorer.UI.Widgets
{
    public class ComponentList : ButtonListHandler<Component, ComponentCell>
    {
        ...
        public override void OnCellBorrowed(ComponentCell cell)
        {
            ...
            cell.OnDumpClicked += OnDumpClicked;
        }
        ...
        private void OnDumpClicked(int index)
        {
            try
            {
                List<Component> entries = GetEntries();
                Component comp = entries[index];
                UERuntimeHelper.DumpComponentData(comp);
            }
            catch (Exception ex)
            {
                ExplorerCore.LogWarning($"Exception dumping Component: {ex.ReflectionExToString()}");
            }
        }
